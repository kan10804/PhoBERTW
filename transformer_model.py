...
class BertAbsSum(nn.Module):
    def __init__(self, config, device):
        ...
        if getattr(Params, 'mode', 'train') == 'train':
    ...
    def forward(self, batch_src_seq, batch_src_mask, batch_tgt_seq, batch_tgt_mask):
        ...
        # shift right (remove last token for teacher forcing)
    ...
    def batch_encode_src_seq(self, batch_src_seq, batch_src_mask):
        """
        Encode input using 1-window (normal) hoặc 2-window (PhoBERT > 256 tokens).
        FIXED: không gọi encoder nếu window rỗng -> tránh lỗi hidden_states[:,0]
        """

        # Ensure correct dtype / device for masks and inputs (they should already be on device)
        # But don't forcibly move here in case caller handles device placement.

        bert_model_name = Params.bert_model.lower() if isinstance(Params.bert_model, str) else ''
        if 'phobert' in bert_model_name and Params.max_src_len > 256:
            # Two-window logic (window 1 + window 2), but skip window 2 if empty
            window_size = 256
            ...
            # Encode window 1 (should be non-empty when max_src_len > 256)
            batch_enc_output1 = self.encoder(input_ids=batch_src_seq1, attention_mask=batch_src_mask1)[0]

            # Encode window 2 only if it has positive length
            if batch_src_seq2.size(1) > 0:
                batch_enc_output2 = self.encoder(input_ids=batch_src_seq2, attention_mask=batch_src_mask2)[0]
                batch_enc_output = torch.cat([batch_enc_output1, batch_enc_output2], dim=1)
            else:
                batch_enc_output = batch_enc_output1
        else:
            # Standard single-window encoding
            batch_enc_output = self.encoder(input_ids=batch_src_seq, attention_mask=batch_src_mask)[0]
    ...
    def greedy_decode(self, batch):
        ...
        batch_enc_output = self.batch_encode_src_seq(batch_src_seq=batch_src_seq, batch_src_mask=batch_src_mask)	# [batch_size, seq_len, hidden_size]	
        batch_dec_seq = torch.full((batch_src_seq.size(0),), Constants.BOS, dtype=torch.long, device=self.device)
        ...
        for idx in range(batch_size):
            ...
            for step in range(Constants.MAX_TGT_SEQ_LEN):
                ...
                for score, node in beams:
                    ...
                    while candidate_count < beam_size and i < sorted_indices.size(0):
                        ...
                        # Block ngram repeats
                        if getattr(Params, 'block_ngram_repeat', 0) > 0:
                            ...
                            # Add heavy penalty to this hypothesis to prevent it from expanding its path
                            if has_repeats:
                        ...
                        if decoded_token == Constants.EOS:
                            ...
                            if getattr(Params, 'min_tgt_len', 0) > 0:
...
class BeamSearchNode(object):
    ...
    # Get beam score with Wu's length normalization (https://arxiv.org/abs/1609.08144)
    def eval(self):
        score = self.log_prob
        ...
        length_norm = length_norm ** getattr(Params, 'len_norm_factor', 0)
